...
export default function MaisVendidos() {
  ...
  const topProducts = useMemo(() => {
    const markedAsBestSeller = products.filter(
      (product) => product.isBestSeller,
    );

    const source =
      markedAsBestSeller.length > 0
        ? markedAsBestSeller
        : products;

    return sortProducts(source, "mais-vendidos").slice(
      0,
      12,
    );
  }, [products]);
  ...
  return (
    <CategoryPageLayout>
      <SectionHeader
        title="Mais vendidos"
        description="Confira os produtos mais populares do catálogo"
      />
          ...
          {loading ? (
            ...
            <div className="flex min-h-[360px] flex-col items-center justify-center rounded-3xl border border-dashed border-border bg-muted/30 px-6 py-12 text-center">
              ...
              <p className="mt-3 max-w-md text-sm leading-6 text-muted-foreground sm:text-base">
                Assim que novos produtos forem cadastrados e
                destacados, eles aparecerão nesta página.
              </p>
