...
const lum = rgb => rgb.map(v => { const c = v / 255; return c <= 0.03928 ? c / 12.92 : Math.pow((c + 0.055)/1.055, 2.4); }).reduce((sum, val, i) => sum + val * [0.2126, 0.7152, 0.0722][i], 0);
...
const accent = hslToRgb(44,83,26);
const accentBg10 = mix(white, accent, 0.1);
console.log('accent hsl(44 83% 26%) white', contrast(accent, white), 'bg10', contrast(accent, accentBg10));
const results = [];
  ...
  for(let s=20; s<=55; s += 1) {
    for(let l=30; l<=48; l += 1) {
      ...
      if(cWhite >= 4.5 && cBg10 >= 4.5) {
        results.push({h,s,l,cWhite,cBg10, rgb});
      }
    }
  }
}
